...
export function useTableData({
  ...
  const handleDirectPaste = useCallback(
    (text: string) => {
      ...
      } else if (result.newRowsToImport) {
        if (result.overrideStartRowId !== undefined) {
          handleOverrideRows(result.overrideStartRowId, result.newRowsToImport);
        } else {
          handleImportRows(result.newRowsToImport);
        }
      }
      ...
      if (result.message) {
        setSnackbar({
          ...
          severity: result.severity ?? "success",
        });
      }
    },
  ...
  useEffect(() => {
    ...
    const markActive = () => setActiveTableContainer(container);
